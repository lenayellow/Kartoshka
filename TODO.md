...
### [Sprint 4.3 → Sprint 6] Android push-уведомления
Перенесено в Sprint 6 — зависит от прохождения модерации в RuStore Console.
...
### Android push-уведомления (RuStore Universal Push)

Использовать RuStore Universal Push SDK (`ru.rustore.sdk:universalpush`) — поддерживает несколько провайдеров одним API. Сейчас подключаем только `RuStorePushProvider`; `FirebasePushProvider` можно добавить позже когда/если приложение будет публиковаться в Google Play.

**Зависимости:** app должен быть загружен в RuStore Console и пройти модерацию.

Шаги:
1. Получить Project ID из RuStore Console (Push notifications → Projects)
2. Подключить `ru.rustore.sdk:universalpush` + `universalrustore` в `build.gradle.kts`
3. Создать `KartoshkaApplication.onCreate` с `RuStoreUniversalPushClient.init`
4. Создать `KartoshkaMessagingService` — наследник `RuStoreMessagingService`
5. Зарегистрировать service в `AndroidManifest` с intent-filter `ru.rustore.sdk.pushclient.MESSAGING_EVENT`
6. Запрашивать runtime `POST_NOTIFICATIONS` на Android 13+
7. Получать токен и отправлять на `POST /users/me/push-token` (бэкенд готов)

Бэкенд: `notifications/rustore.go` готов.
